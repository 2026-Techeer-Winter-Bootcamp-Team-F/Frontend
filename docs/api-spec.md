...
> 금액 내림차순 정렬. 전체 13개 카테고리 반환. `change`는 전월 대비 증감액 (양수=증가, 0=동일). `percent`는 정수로 반올림되어 합계가 정확히 100%가 아닐 수 있음 (실측: 93%).
...
`category-summary`의 `percent` 값 합계가 93%입니다 (정수 반올림으로 인한 차이).
